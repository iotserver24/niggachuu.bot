...
def has_joined_required_groups(user_id):
    try:
        auction_status = bot.get_chat_member(chat_id="@AVSAUCTION", user_id=user_id).status
        trade_status = bot.get_chat_member(chat_id="@Avstradegc", user_id=user_id).status
...
def send_welcome_message(chat_id, username, first_name):
    ...
    join_auction_btn = types.InlineKeyboardButton("Join Auction", url="https://t.me/AVSAUCTION")
    join_trade_btn = types.InlineKeyboardButton("Join Trade", url="https://t.me/Avstradegc")
...
DATABASE_NAME = "auction_items"
COLLECTION_NAME = "items"
GROUP_CHAT_ID = -1002073133163
ADMIN_CHAT_ID = [7387719195, 6541873974]

client = pymongo.MongoClient(MONGO_URI)
db = client[DATABASE_NAME]
collection = db[COLLECTION_NAME]

@bot.message_handler(commands=['add'], func=lambda message: message.chat.type == 'private')
def send_add_options(message):
    if is_admin(message.from_user.id) or is_owner(message.from_user.id):
        markup = telebot.types.InlineKeyboardMarkup(row_width=2)
        markup.add(telebot.types.InlineKeyboardButton("Pokemon", callback_data="add_pokemon"),
                   telebot.types.InlineKeyboardButton("TMs", callback_data="add_tms"),
                   telebot.types.InlineKeyboardButton("Teams", callback_data="add_teams"))
...
def get_pokemon_name(message):
    name = message.text
    if not name:
        message.reply_text("<b>Please send text only.</b>", parse_mode="HTML")
        return get_pokemon_name(message)
...
def get_pokemon_picture(message, name):
    if not message.photo:
        message.reply_text("<b>Please send only a photo.</b>", parse_mode="HTML")
        return get_pokemon_picture(message, name)
...
def get_pokemon_info(message, name, picture):
    ...
    if not info:
        message.reply_text("<b>Please send text only.</b>", parse_mode="HTML")
        return get_pokemon_info(message, name, picture)
    bot.send_message(message.chat.id, "<b>Provide the Pokemon's IV/EV stats:</b>", parse_mode="HTML")
    bot.register_next_step_handler(message, get_pokemon_ivev, name, picture, info)

def get_pokemon_ivev(message, name, picture, info):
    ivev = message.text
    if not ivev:
        message.reply_text("<b>Please send text only.</b>", parse_mode="HTML")
        return get_pokemon_ivev(message, name, picture, info)
    bot.send_message(message.chat.id, "<b>Provide the Pokemon's moveset:</b>", parse_mode="HTML")
    bot.register_next_step_handler(message, get_pokemon_moveset, name, picture, info, ivev)

def get_pokemon_moveset(message, name, picture, info, ivev):
    moveset = message.text
    if not moveset:
        message.reply_text("<b>Please send text only.</b>", parse_mode="HTML")
        return get_pokemon_moveset(message, name, picture, info, ivev)
    bot.send_message(message.chat.id, "<b>Is It Boosted ?:</b>", parse_mode="HTML")
    bot.register_next_step_handler(message, get_pokemon_boosted, name, picture, info, ivev, moveset)

def get_pokemon_boosted(message, name, picture, info, ivev, moveset):
    boosted = message.text
    if not boosted:
        message.reply_text("<b>Please send text only.</b>", parse_mode="HTML")
        return get_pokemon_boosted(message, name, picture, info, ivev, moveset)
    bot.send_message(message.chat.id, "<b>Provide the Pokemon's base :</b>", parse_mode="HTML")
    bot.register_next_step_handler(message, get_pokemon_base, name, picture, info, ivev, moveset, boosted)

def get_pokemon_base(message, name, picture, info, ivev, moveset, boosted):
    base = message.text
    if not base:
        message.reply_text("<b>Please send text only.</b>", parse_mode="HTML")
        return get_pokemon_base(message, name, picture, info, ivev, moveset, boosted)
    user_id = message.from_user.id
    ...
    markup.add(telebot.types.InlineKeyboardButton("Approve", callback_data=f"approve_pokemon_{user_id}_{name}_{user_name}"),
               telebot.types.InlineKeyboardButton("Reject", callback_data=f"reject_pokemon_{user_id}_{name}"))
...
def get_tms_info(message, name):
    ...
    if not info:
        message.reply_text("<b>Please send text only.</b>", parse_mode="HTML")
        return get_tms_info(message, name)
...
def get_tms_base(message, name, info):
    ...
    if not base:
        message.reply_text("<b>Please send text only.</b>", parse_mode="HTML")
        return get_tms_base(message, name, info)
    user_id = message.from_user.id
    user_name = message.from_user.username

    caption = f"<b>TMs Name:</b> {name}\n<b>TMs Info:</b> {info}\n<b>Base:</b> {base}\n<b>UserID:</b> {user_id}"
    markup = telebot.types.InlineKeyboardMarkup()
    markup.add(telebot.types.InlineKeyboardButton("Approve", callback_data=f"approve_tms_{user_id}_{user_name}"),
               telebot.types.InlineKeyboardButton("Reject", callback_data=f"reject_tms_{user_id}"))
...
def handle_tms_approval_rejection(call):
    ...
    if action == "approve":
        ...
        collection.insert_one({"type": "tms", "name": user_name})
    else:
...
def get_team_members(message, name):
    ...
    if not members:
        message.reply_text("<b>Please send text only.</b>", parse_mode="HTML")
        return get_team_members(message, name)
...
def get_team_base(message, name, members):
    ...
    if not base:
        message.reply_text("<b>Please send text only.</b>", parse_mode="HTML")
        return get_team_base(message, name, members)
    user_id = message.from_user.id
    user_name = message.from_user.username

    caption = f"<b>Team Name : </b> {name}\n\n<b>Members : \n\n</b> {members}\n\n<b>Base : </b> {base}\n\n<b>UserID : </b> {user_id}"
    markup = telebot.types.InlineKeyboardMarkup()
    markup.add(telebot.types.InlineKeyboardButton("Approve", callback_data=f"approve_team_{user_id}_{name}_{user_name}"),
               telebot.types.InlineKeyboardButton("Reject", callback_data=f"reject_team_{user_id}_{name}"))
...
def handle_approval_rejection(call):
    ...
    if item_type == "pokemon":
        if action == "approve":
            ...
            collection.insert_one({"type": "pokemon", "name": item_name})
        else:
        ...
        if action == "approve":
            bot.send_message(user_id, "<b>Your TMs submission has been approved!</b>", parse_mode="HTML")
            bot.send_message(GROUP_CHAT_ID, f"<b>New TMs added:</b>\n{call.message.text}", parse_mode="HTML")
            collection.insert_one({"type": "tms", "name": item_name})
        else:
            bot.send_message(user_id, "<b>Your TMs submission has been rejected.</b>", parse_mode="HTML")
    elif item_type == "team":
        if action == "approve":
            ...
            collection.insert_one({"type": "team", "name": item_name})
        else:
            bot.send_message(user_id, f"<b>Your Team '{item_name}' has been rejected.</b>", parse_mode="HTML")
    
    bot.edit_message_reply_markup(call.message.chat.id, call.message.message_id, reply_markup=None)

@bot.message_handler(commands=['current'], func=lambda message: message.chat.type == 'private')
def current_items(message):
    if is_admin(message.from_user.id) or is_owner(message.from_user.id):
        pokemon_count = collection.count_documents({"type": "pokemon"})
        tms_count = collection.count_documents({"type": "tms"})
        teams_count = collection.count_documents({"type": "team"})
...
@bot.message_handler(commands=['item'], func=lambda message: message.chat.type == 'private')
def show_item_options(message):
    if is_admin(message.from_user.id) or is_owner(message.from_user.id):
        markup = telebot.types.InlineKeyboardMarkup(row_width=3)
        markup.add(telebot.types.InlineKeyboardButton("Pokemon", callback_data="show_pokemon"),
                   telebot.types.InlineKeyboardButton("TMs", callback_data="show_tms"),
                   telebot.types.InlineKeyboardButton("Teams", callback_data="show_teams"))
...
def handle_show_items(call):
    if is_admin(call.from_user.id) or is_owner(call.from_user.id):
        ...
        items_list = list(collection.find({"type": item_type.lower()}))
...
@bot.message_handler(commands=['list'], func=lambda message: message.chat.type == 'private')
    ...
    if is_admin(message.from_user.id) or is_owner(message.from_user.id):
        ...
        if items_list:
            ...
            for i, item in enumerate(items_list):
                items_message += f"{i + 1}. {item['type'].capitalize()}: {item['name']}\n"
        else:
...
@bot.message_handler(commands=['clear'], func=lambda message: message.chat.type == 'private')
